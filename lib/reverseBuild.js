...
module.exports = function buildTemplate(filename){
  ...
  //read template
  if(filename.includes(".html")){
    fs.readFile(`./site/${filename}`, 'utf8', function(err, originFile){
      if(err) throw new Error(err)
        ...
        if(originFile.includes('<!-- <')){
          ...
          for(let inst of objArray){
            ...
            if(originFile.includes(start_tag)){
              ...
              fs.writeFileSync(`./toruf/objects/${inst}.json`, dbString, 'utf8')
        ...
        for(let comp of compArray){
          ...
          if(originFile.includes(`<!-- ${tag} -->`)){
            ...
            //save component
            fs.writeFileSync(`./toruf/components/${comp}.html`, component)
            let piece = `<!-- ${tag} -->` + component + `<!-- !${tag} -->`;
            originFile = originFile.replace(piece, tag);
          }
        }
        //save file
        fs.writeFileSync(`./toruf/templates/${filename}`, originFile, 'utf8')
      }
    })
  }
}
